...
const Game = ({ games }: any) => {
  ...
  useEffect(() => {
    if (window.innerWidth <= 700 && totalCards > 5&&!isauto) {
  ...
  useEffect(() => {
    if (isauto === true) {
      ...
      if (totalCards > 1 && window.innerWidth > 700) {
        const intervalId = setInterval(() => {
          moveRight();
        }, 3000);
  ...
  return (
    <div
      className="h-[45dvh] sm:h-[40dvw] overflow-x-scroll sm:overflow-x-hidden overflow-y-hidden transition-all element w-100vw"
    ...
    >
      {!open ? (
        <div
          ref={containerRef}
          className="flex w-full justify-evenly items-center element overflow-x-scroll sm:overflow-x-hidden sm:overflow-y-hidden transition-all mx-auto relative h-full"
        >
